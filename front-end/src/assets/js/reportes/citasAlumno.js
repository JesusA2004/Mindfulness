...
export async function getCitasAlumno(f = {}) { return fetchJSON(ENDPOINT, buildQuery(f)); }
...
export async function openCitasAlumnoDialog(tipo = "pdf") {
  ensureSwalStyles();
  // Reusa el mismo modal de actividades (alumno + rango)
  // (para mantener independencia, copiamos la plantilla mínima)
  const html = `
  <div class="container-fluid p-0">
    <div class="sw-head"><i class="${tipo==='excel'?'bi bi-filetype-xlsx':'bi bi-filetype-pdf'} text-primary"></i>
      <div>Rango y (opcional) alumno</div></div>
    <div class="row g-3">
      ...
      <div class="col-12 col-md-6">
        <div class="sw-section h-100">
          ...
          <div class="sw-list" id="sw-alumno-list"></div>
        </div>
      </div>
    </div>
  </div>`;
  ...
  const res = await Swal.fire({
    ...
    didOpen: async () => {
      ...
      const render = (list=[]) => {
        if (!list.length) { $alist.innerHTML = `<div class="mini-hint">Escribe y presiona <b>Enter</b>.</div>`; return; }
        $alist.innerHTML = list.map((o,i)=>`<button type="button" class="sw-item" data-label="${o.label}" data-idx="${i}">
          <span class="nm">${o.nombre}</span><span class="mt">${o.matricula}</span></button>`).join("");
        Array.from($alist.querySelectorAll(".sw-item")).forEach((el)=> {
          el.addEventListener("click", ()=>{ $alist.querySelectorAll(".sw-item").forEach(n=>n.classList.remove("active")); el.classList.add("active"); });
        });
      };

      await loadAllAlumnos(); render(alumnosCache);
      const doFilter = debounce((t)=>render(filterAlumnosLocal((t||"").trim())),140);
      $ainput.addEventListener("input",(e)=>doFilter(e.target.value||""));
  ...
  try {
    const { blob, filename } = await exportCitasAlumno(tipo, res.value || {});
