...
export async function getRecompensasCanjeadas(f = {}) { return fetchJSON(ENDPOINT, buildQuery(f)); }
...
export async function openRecompensasCanjeadasDialog(tipo = "pdf") {
  ...
  <div class="container-fluid p-0">
    <div class="sw-head"><i class="${tipo==='excel'?'bi bi-filetype-xlsx':'bi bi-filetype-pdf'} text-primary"></i>
      <div>Rango y (opcional) tipo de recompensa</div></div>
    <div class="row g-3">
      ...
      <div class="col-12 col-md-6">
        <div class="sw-section h-100">
          <label class="form-label mb-2">Tipo de recompensa</label>
          <input id="sw-tipo-input" class="form-control mb-2" type="text" placeholder="(vacío = todas)">
        </div>
      </div>
    </div>
  </div>`;
  ...
  const res = await Swal.fire({
    ...
    preConfirm: () => {
      ...
      const v = ($t?.value || "").trim(); if (v) filtros.tipo = v;
      return filtros;
    }
  });
  ...
  try {
    const { blob, filename } = await exportRecompensasCanjeadas(tipo, res.value || {});
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob);
    a.download = filename || `recompensas_${new Date().toISOString().slice(0,19).replace(/[:T]/g,"-")}.${tipo==="excel"?"xlsx":"pdf"}`;
