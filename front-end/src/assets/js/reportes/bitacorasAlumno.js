...
export async function getBitacorasAlumno(f = {}) { return fetchJSON(ENDPOINT, buildQuery(f)); }
...
export async function openBitacorasAlumnoDialog(tipo = "pdf") {
  ensureSwalStyles();
  // Igual a actividades/citas, solo cambia el título
  const html = `
  ...
  try {
    const { blob, filename } = await exportBitacorasAlumno(tipo, res.value || {});
