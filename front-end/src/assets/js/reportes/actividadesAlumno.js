...
async function previewTable(filtros = {}) {
  ...
  const chips = [
    `<span class="chip"><b>Rango:</b> ${filtros.desde && filtros.hasta ? `${filtros.desde} a ${filtros.hasta}` : "Todas las fechas"}</span>`,
    filtros.alumno ? `<span class="chip"><b>Alumno:</b> ${String(alumnoChip)}</span>` : ""
  ].filter(Boolean).join("");

  const head = `
    <tr>
      <th>alumno</th><th>matricula</th><th>tecnica</th><th>fecha</th><th>titulo</th><th>descripcion</th>
    </tr>`;
  ...
  const html = `
    ...
    <div class="pv-card">
      <table>
        <thead>${head}</thead>
        <tbody>${body || `<tr><td colspan="6" class="text-muted">Sin datos en el rango seleccionado.</td></tr>`}</tbody>
      </table>
...
export async function openActividadesAlumnoDialog(tipo = "pdf") {
  ...
  try {
    const { blob, filename } = await exportActividadesAlumno(tipo, filtros);
