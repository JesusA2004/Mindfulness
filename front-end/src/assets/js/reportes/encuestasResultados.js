...
async function previewChart(filtros = {}) {
  ...
  const chartData = asArray(json?.meta?.chartData).length
    ...
    : labels.map((l,i) => {
        ...
        return { label:String(l||"Encuesta"), value:v, pct: total? Math.round((v/total)*1000)/10 : 0 };
      });

  const chips = [
    `<span class="chip"><b>Rango:</b> ${json?.meta?.rango || (filtros.desde && filtros.hasta ? `${filtros.desde} a ${filtros.hasta}` : 'Todas las fechas')}</span>`
  ].join('');
  ...
  const html = `
    <style>
      ...
      .pv-grid{white-space:nowrap;text-align:center;border-bottom:1px solid #e5e7eb;padding:8px 6px 12px 6px;font-size:0}
      .bar-col{display:inline-block;vertical-align:bottom;width:22%;margin:0 1.5%;font-size:12px}
    ...
    <div class="pv-wrap">
      ...
      <div class="pv-total">Total de respuestas: <b>${total}</b></div>
    </div>`;
