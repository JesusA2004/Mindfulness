<!-- src/views/<rol>/SeguimientoCitas.vue -->
<template>
...
/* Datos */
const all = ref([])
const allDecorated = computed(() => all.value.map(decorate))
...
const filtered = computed(() => {
  ...
  return allDecorated.value.filter(c => {
    // alumno + maestro + motivo + modalidad/estado
...
function buildPutPayload (cita, override = {}) {
  // Usa los valores actuales de la cita y permite overrides (estado/observaciones)
  return {
    alumno_id: alumnoIdOf(cita),
    docente_id: docenteIdOf(cita),
    fecha_cita: cita.fecha_cita, // ya viene ISO del backend
...
/* Cambiar estado: Aceptar / Rechazar (PUT completo + fallback PATCH) */
async function cambiarEstado (cita, nuevoEstado) {
  ...
  try {
    ...
    // Fallback: si tuvieras ruta PATCH /citas/{id}/estado
