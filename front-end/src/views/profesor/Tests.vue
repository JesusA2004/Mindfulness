<!-- src/views/docente/TestsEmocionales.vue -->
<template>
...
const {
  items, isLoading, hasMore, filteredItems, /* <- filtro por texto (nombre/descr.) que ya tenías */
...
/* ==========
   Filtros por fecha (locales en el componente)
=========== */
...
function normalizeDate(value) {
  ...
  try {
    // Si viene como ISO o Date, lo normalizamos a YYYY-MM-DD
...
/* Items a mostrar = resultado de filtro por texto (filteredItems) + filtro por fecha */
const displayedItems = computed(() => {
  ...
  return list.filter(it => {
    ...
    if (!d) return false; // si no tiene fecha, no entra cuando filtramos por fecha
...
/* Preguntas del test seleccionado, con filtro local por texto */
...
/* Cantidad visible para la barra de progreso */
const visibleQuestions = computed(() => filteredQuestions.value.length);

/* Mapeo de índice visible -> índice real del array original (para usar viewToggle.qOpen) */
...
/* Acciones expandir/contraer todo según el universo visible */
const expandAll = () => {
  ensureQOpenSize();
...
/* Asegura longitud de arreglo qOpen */
